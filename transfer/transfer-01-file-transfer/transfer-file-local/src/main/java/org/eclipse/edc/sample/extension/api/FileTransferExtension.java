...
public class FileTransferExtension implements ServiceExtension {
    ...
    private void registerDataEntries(ServiceExtensionContext context) {
        ...
        var asset = Asset.Builder.newInstance()
                ...
                .version("1.0")
                .contentType("txt").build();

        assetIndex.accept(asset, dataAddress);
        context.getMonitor().info("===================================== asset registered =================================");

        var asset2 = assetIndex.findById("test-document");
        context.getMonitor().info(asset2.toString());
        context.getMonitor().info(format("%s :: Received asset ID", asset2.getId()));
        context.getMonitor().info(format("%s :: Received asset Name", asset2.getName()));
        context.getMonitor().info(format("%s :: Received asset Description", asset2.getDescription()));
        context.getMonitor().info(format("%s :: Received asset Version", asset2.getVersion()));
        context.getMonitor().info(format("%s :: Received asset Type", asset2.getContentType()));
    }
